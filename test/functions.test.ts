...
{ // Fn
    ...
    { // Fn<[infer First], infer R>
        ...
        type Actual = Testee extends Fn<[infer First], infer R> ? [First, R] : never;
    ...
    { // Fn<[infer First, ...infer Rest], infer R>
        type Testee = (a: number) => string;
        type Actual = Testee extends Fn<[infer First, ...infer Rest], infer R> ? [First, Rest, R] : never;
        type Expected = [number, [], string];
        assertType<Is<Actual, Expected>>();
    }
